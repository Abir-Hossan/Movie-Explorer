    ...
    <article className="group overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/70 shadow-xl shadow-black/10 transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:shadow-2xl">
      <div className="relative aspect-[2/3] overflow-hidden bg-zinc-800">
        ...
        <div className="absolute inset-x-0 top-0 flex justify-between p-3">
          <span className="rounded-full border border-white/10 bg-black/60 px-2.5 py-1 text-xs font-semibold backdrop-blur">
            {movie.status || "Show"}
          </span>
          <span className="inline-flex items-center gap-1 rounded-full bg-black/70 px-2.5 py-1 text-xs font-semibold backdrop-blur">
      ...
      <div className="p-4">
        ...
        <p className="mt-3 line-clamp-2 min-h-10 text-xs leading-5 text-zinc-500">
          {stripHtml(movie.summary) || "No summary available."}
        </p>
        <button
          onClick={() => onDetails(movie)}
          className="mt-4 w-full rounded-xl bg-white/8 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-red-600"
        >
          See Details
        </button>
      </div>
